...
const Home = () => {
  return (
    <main className="grid grid-cols-1 pt-[56px] px-4 bg-slate-50 ">
      <section className="h-[calc(100dvh-56px)] p-12 flex flex-row items-center max-md:flex-col  max-md:items-center gap-7">
        <div className="flex flex-col ">
          <div className="flex flex-col max-md:items-center pb-14">
            <h2 className="text-slate-800  leading-[46px] text-3xl font-bold ">{FLINE_INTRODUCE}</h2>
            <h2 className="text-slate-800  leading-[46px] text-3xl font-bold indent-[2em]">
              我是
            ...
            <h2 className="text-slate-800  leading-[46px] text-3xl font-bold indent-[3em]">{TLINE_INTRODUCE}</h2>
          </div>
          <p className="text-slate-500 text-sm indent-[2em] ">{BLOG_INTRODUCE}</p>
        </div>
        <div className="flex flex-col min-w-72 items-center flex-1">
          <Avatar className="h-60 w-60 flex flex-col">
            <AvatarImage src={AVATAR_URL} />
            <AvatarFallback>{NICKNAME}</AvatarFallback>
          </Avatar>
          <span className="text-center leading-10 text-slate-800">{NICKNAME}</span>

          <ul className="flex gap-5 pt-6">
            <li>
              <Link href={GITHUB_URL}>
                <Button variant={'ghost'} size="icon" className="rounded-full">
                  <Github className="stroke-slate-950 w-6 h-6" />
                </Button>
              </Link>
            </li>
            <li>
              <Link href={JUEJIN_URL}>
                <Button variant={'ghost'} size="icon" className="rounded-full">
                  <svg
                    ...
                    viewBox="0 0 33 28"
                    fill="none"
                    className="fill-slate-950 w-6 h-6"
                  >
                    <path
      ...
      <section className="h-screen flex max-md:flex-col max-h-screen p-12 ">
        <div className="bg-white w-full rounded-lg px-10">
          <h3 className="leading-[4rem] text-center text-2xl border-b border-b-slate-100">推荐文章👍</h3>

          <ul className="grid grid-cols-2 grid-rows-2 px-4 py-8 gap-16">
            <li>
              <Link href={'/'}>
                <h3 className="text-slate-800 leading-10">NextAuth实现微信登录</h3>
                <p className="text-slate-500 text-sm line-clamp-4 border-b border-b-slate-100 leading-5 pb-2">
                  Lorem ipsum dolor sit amet consectetur adipisicing elit. Blanditiis voluptatum, harum nam commodi a
                  laudantium perspiciatis corrupti eligendi mollitia praesentium enim odit sit at. Fugit dolores
                  dignissimos tempore delectus id?
                </p>
              </Link>
            </li>
            <li>
      ...
      <section className="h-screen flex max-md:flex-col max-h-screen p-12 ">
        <div className="bg-white w-full rounded-lg px-10">
          <h3 className="leading-[4rem] text-left text-2xl border-b border-b-slate-100 ">关于我😊</h3>
          <p className="pt-4 text-slate-500 text-sm indent-[2em] leading-6 ">
